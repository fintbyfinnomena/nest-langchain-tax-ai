...
export async function generatorAgentNode(params: {
  ...
  const agent = await createOpenAIAgent(llm, tools, systemPrompt);
  ...
  const agentNode = async (
    ...
    return {
      messages: [new AIMessage({ content: result.output, name })],
    };
  };
...
async function promptMessageSequenceGenerate(
  systemPrompt: string,
  chatHistory?: boolean,
): Promise<any> {
  let prompt: any;
